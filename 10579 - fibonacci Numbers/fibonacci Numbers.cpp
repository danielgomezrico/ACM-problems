/* 
   ...
   http://icpcres.ecs.baylor.edu/onlinejudge/index.php?option=com_onlinejudge&Itemid=8&category=17&page=show_problem&problem=1520
*/
...
{
    int m;
    int sizeVFibs = 0;//Contiene el size de vFibs para no tener que recontarlos cada vez que se nesesite el mismo tamaño
    ...
    while(cin >>m ){
       ...
       if(sizeVFibs <= m){//Si el numero ya se agrego, si no entra
            ...
            if(m == 1 || m == 2){
                ...
                cout << sumar(vFibs[m-2],vFibs[m-1])<<endl;
            }
       }
    }
    return 0;
